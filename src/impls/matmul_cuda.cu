...
template <const int BM, const int BN, const int BK, const int WM, const int WN, const int CHUNK_K,
          ...
          const int WMMA_N,
          typename OutputType>
__global__ void matmul_kernel_Tensor_v7(const half *A, const half *B, OutputType *C, size_t M, size_t K, size_t N)
{
    using namespace nvcuda;
...
void matmulTensorCores(InputType *A, InputType *B, OutputType *C, size_t M, size_t K, size_t N)
{
    ...
    else if constexpr (version == 6 || version == 7)
    {
        if (M < 256 | K < 256 | N < 256)
        {
            constexpr struct matmulTemplateArgsTensor p = getMatmulTemplateArgsTensor<0, FRAG_SIZE_M, FRAG_SIZE_K, FRAG_SIZE_N>();
            ...
            {
                matmul_kernel_Tensor_v7<p.BM, p.BN, p.BK, p.WM, p.WN, p.CHUNK_K, p.N_WARP_ROWS_PER_BLOCK, p.N_WARP_COLS_PER_BLOCK, p.N_WMMA_ROWS_PER_WARP, p.N_WMMA_COLS_PER_WARP, FRAG_SIZE_M, FRAG_SIZE_K, FRAG_SIZE_N, OutputType>
                    <<<blocks, p.threadsPerBlock, smemSize>>>(A, B, C, M, K, N);
            }
        }
        else
        {
            ...
            {
                cudaFuncSetAttribute(matmul_kernel_Tensor_v7<p.BM, p.BN, p.BK, p.WM, p.WN, p.CHUNK_K, p.N_WARP_ROWS_PER_BLOCK, p.N_WARP_COLS_PER_BLOCK, p.N_WMMA_ROWS_PER_WARP, p.N_WMMA_COLS_PER_WARP, FRAG_SIZE_M, FRAG_SIZE_K, FRAG_SIZE_N, OutputType>, cudaFuncAttributeMaxDynamicSharedMemorySize, smemSize);
                matmul_kernel_Tensor_v7<p.BM, p.BN, p.BK, p.WM, p.WN, p.CHUNK_K, p.N_WARP_ROWS_PER_BLOCK, p.N_WARP_COLS_PER_BLOCK, p.N_WMMA_ROWS_PER_WARP, p.N_WMMA_COLS_PER_WARP, FRAG_SIZE_M, FRAG_SIZE_K, FRAG_SIZE_N, OutputType>
                    <<<blocks, p.threadsPerBlock, smemSize>>>(A, B, C, M, K, N);
            }
        }
...
template void matmulTensorCores<half, float, 7>(half*, half*, float*, size_t, size_t, size_t);
#endif
